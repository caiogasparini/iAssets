...
function Expenses({data}) {
    ...
    const [totalText, setTotalText] = useState('Despesas Totais');
    ...
    function closeFormEdit() {
        ...
        //zerando valores para o form Edit
        setExpenseName('');
        setObs('');
        setDueDate('');
        setDatePayment('');
    ...
    function somaCalc (d) {
        ...
        if (d.length > 0) {
            ...
            r += '';
            let result = (r.length - 1) - r.indexOf('.');
            if (result > 2) {
                let re = r.indexOf('.') + 3;
                r = r.slice(0,re);
                }
    ...
    useEffect( () => {
        //console.log(data)
        let result = somaCalc(data);
        setTotalCalc('R$ ' + result);
    },[data])
    ...
    async function handleSave(e, data) {
        ...
        if (response) {
            if (response.status === 200) {
                ...
                setUpdateJSON(JSONFiltered[0]._id,response.data);
    ...
    async function handleSubmit(e) {
        ...
        if (response) {
            if (response.status === 200) {
                ...
                document.getElementById('http-add').style.display = 'flex';
                document.getElementById('http-add').innerText = 'Salvo com sucesso!'
            }
            setTimeout(() => {
                document.getElementById('http-add').style.display = 'none';
                //console.log(response.status)
            }, 3000);
        ...
        <>
            <div className="conteiner-expenses">
                <div className="conteiner-list">
                    ...
                    <div className="list">
                        <ul className="u-list">
                            {allData.map( data => (<li className="list-cards" key={data._id} onClick={() => openFormEdit(data)}><ExpenseCards data={data}/></li>))}
                        </ul>
            ...
            <div className="conteiner-viewer">
                        ...
                        <div className="card-body content-color">
                            ...
                            <div className="somas">
                                <div className="total-expense">
                                    ...
                                    <div className="te-value text-center">{calc}</div>
                                </div>
                                <div className="parcial-expense"> 
                                    <div className="te-text text-center">{totalText}</div>
                                    <div className="te-value text-center">{totalCalc}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="card-viewer">
                    ...
                    <div className="card-detail" id="cd-add">
                        <form className="card border-orange bg-base-primary mb-3" onSubmit={handleSubmit}>
                            ...
                            <div className="d-flex">
                                ...
                                <div className="card-header div-buttons d-flex justify-content-around align-items-center">
                                    <button className="btn btn-sm btn-outline-orange" type="submit">Adicionar</button>
                                </div>
                            </div>
                            <div className="card-body content-color body-viewer">
                                ...
                                <div className="detail-conteiner">
                                    <div className="dates">
                                        ...
                                        <input className="content-due" type="date" onChange={e => setDueDate(e.target.value)} value={dueDate} required/>
                                        </div>
                                    ...
                                    <div className="value">
                                        ...
                                        <input className="content-valor" id="add-value" type="number" placeholder='0,00' onChange={(props) => maskCurrency(props.target.id)} value={value} required/>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="card-detail" id="cd-edit">
                        ...
                        <div className="msg-http" id="http-edit">Salvo com sucesso!</div>
                            <div className="d-flex">
                                ...
                                <div className="card-header div-buttons d-flex justify-content-around align-items-center">
                                <button className="btn btn-sm btn-outline-orange" onClick={e => handleDelete(e.target, allData)} type="button">Descartar</button>
                                <button className="btn btn-sm btn-outline-orange" onClick={e => handleSave(e.target, allData)} type="button">Salvar</button>
                                </div>
                            </div>
                            <div className="card-body content-color body-viewer">
                                ...
                                <div className="detail-conteiner">
                                    <div className="dates">
                                        <div className="due">
                                        <div className="title-due">Data de Vencimento</div>
                                        <input className="content-due" id="due" type="date" onChange={e => setDueDate(e.target.value)} value={dueDate} required/>
                                        </div>
                                        <div className="payment">
                                        <div className="title-payment">Data de Pagamento</div>
                                    ...
                                    <div className="value">
                                        <div className="title-valor">Valor</div>
                                        <div className="div-cv">
                                            <input className="content-valor" id="value" type="number" onChange={(props) => maskCurrency(props.target.id)} value={value} required/>
                                        </div>
                                    </div>
                                </div>
                            </div>
